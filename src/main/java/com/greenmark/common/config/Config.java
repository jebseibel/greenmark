...
import com.greenmark.utils.UTDirectoryHandler;
import com.greenmark.utils.UTDisplayFormatter;
import com.greenmark.utils.UTFileHandler;
...
public class Config {
	 ...
	 */
	public static String lastfilesaved_config = new String();
	public static String lastfilesaved_model = new String();
	...
	public static final void initializeMarketTrends() throws Exception {
		...
		if (marketTrendUseOn == GmConstants.IS_TRUE) {
			...
			if (marketTrendEnableOn == GmConstants.IS_TRUE) {
				Config.setEnabledMktd(true);
			} else {
				Config.setEnabledMktd(false);
			}
		} else {
	...
	public static final boolean getValueAsBoolean(String dvtag) {
		try {
			...
			if (stringvalue.toUpperCase().equals("NO"))
				return false;

			if (stringvalue.toUpperCase().equals("1"))
				return true;

			if (stringvalue.toUpperCase().equals("0"))
				return false;

			if (stringvalue.toUpperCase().equals("TRUE"))
				return true;

			if (stringvalue.toUpperCase().equals("FALSE"))
				return false;
	...
	public static boolean isValidModelXml(DVDatavault dv) throws Exception {
		...
		// see if
		boolean isUseOn = false;
		if (use.equals(GmConstants.STR_TRUE))
			isUseOn = true;

		boolean isEnableOn = false;
		if (enabled.equals(GmConstants.STR_TRUE))
			isEnableOn = true;
