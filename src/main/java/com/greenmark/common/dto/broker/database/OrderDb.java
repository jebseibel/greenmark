...
//		this.compactedExchangeSymbol = myPosition.getCompactedExchangeSymbol();
	...
	public String toXmlWrapper(String prefix, String endline) {
		...
		stb.append(prefix + "<ORDER>" + endline);
	...
	public String toXml(String prefix, String endline) {
		...
		stb.append(prefix + UTDisplayFormatter.TAB + "<ENTRY_PRICE>" + entryPrice + "</ENTRY_PRICE>" + endline);
		stb.append(prefix + UTDisplayFormatter.TAB + "<HIGH_LOW_PRICE_ON_ORDER>" + highLowPriceOnOrder + "</HIGH_LOW_PRICE_ON_ORDER>" + endline);
		stb.append(prefix + UTDisplayFormatter.TAB + "<CURRENT_PRICE>" + currentPrice + "</CURRENT_PRICE>" + endline);

		stb.append(prefix + UTDisplayFormatter.TAB + "<TOTAL_SHARES_REMAINING>" + totalSharesRemaining + "</TOTAL_SHARES_REMAINING>" + endline);

		stb.append(prefix + UTDisplayFormatter.TAB + "<HARVEST_STRATEGY>" + harvestStrategyAcronym + "</HARVEST_STRATEGY>" + endline);
		stb.append(prefix + UTDisplayFormatter.TAB + "<ATTEMPTING_REPLACE>" + attemptingReplaceOfOrder + "</ATTEMPTING_REPLACE>" + endline);
		return stb.toString();
	}
	...
	public LagPeriodsPrice getOrderLagPeriodsPrice() {
		if (entryPriceDatetime != null) {
			...
			if (this.isMomentumUp == 0)
				lagPeriodInfo.setCalcPriceHigher(true);
			else
				lagPeriodInfo.setCalcPriceHigher(false);
