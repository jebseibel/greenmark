...
public class ApplicationDataContext {
	...
	// Save/Restore XML
	public static String lastfilesaved_services = new String();
	...
	public static void initialize(String xmldata) {
		try {
			...
			if ((resultsDbString != null) && (resultsDbString.trim().equals("") == false)) {
			...
			if ((pricesDbString != null) && (pricesDbString.trim().equals("") == false)) {
	...
	public static final String toXml(String prefix, String endline) {
		...
		stb.append("<GLOBAL_EXECUTION_DATA>" + endline);

		stb.append(prefix + "<IS_HISTORICAL_DATAFEED>" + isHistoricalDatafeed + "</IS_HISTORICAL_DATAFEED>" + endline);
		stb.append(prefix + "<IS_CHARTS_DATAFEED>" + isChartsDatafeed + "</IS_CHARTS_DATAFEED>" + endline);

		//
		// add the Selected Results DB Connection information
		stb.append(UTDisplayFormatter.TAB + "<RESULTS_DB_CONNECTION_DATA>" + endline);
		stb.append(resultsDbProperties.toXml(prefix + UTDisplayFormatter.TAB, "\n"));
		stb.append(UTDisplayFormatter.TAB + "</RESULTS_DB_CONNECTION_DATA>" + endline);

		// add the Prices Results DB Connection information
		stb.append(UTDisplayFormatter.TAB + "<PRICES_DB_CONNECTION_DATA>" + endline);
		stb.append(pricesDbProperties.toXml(prefix + UTDisplayFormatter.TAB, "\n"));
		stb.append(UTDisplayFormatter.TAB + "</PRICES_DB_CONNECTION_DATA>" + endline);
	...
	public static boolean isHistoricalScenario() {
		if (isHistoricalDatafeed() || isHistoricalDatafeed_Chart())
			return true;
		else
			return false;
	}
	...
	public static boolean isLiveScenario() {
		if (isLiveDatafeed() || isLiveDatafeed_Chart())
			return true;
		else
			return false;
	}
	...
	public static boolean isLiveDatafeed_Chart() {
		if (!isHistoricalDatafeed() && isChartsDatafeed())
			return true;
		else
			return false;
	}
