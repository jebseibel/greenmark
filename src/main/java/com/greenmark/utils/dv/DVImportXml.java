...
public class DVImportXml {
    ...
    public static final void extract(StringBuffer xml, DVDatavault dv, String dataVaultTag) {
        String col = Character.toString(DVImportXml.COLLECTION_DELIMITER);
        DVImportXml.extract(xml, dv, dataVaultTag, new StringBuffer(col));
    }

    public static final void extract(String xml, DVDatavault dv, String dataVaultTag) {
        StringBuffer stb = new StringBuffer(xml);
        String col = Character.toString(DVImportXml.COLLECTION_DELIMITER);
        DVImportXml.extract(stb, dv, dataVaultTag, new StringBuffer(col));
    }
    ...
    private static final Vector _parse(StringBuffer xml, int vectorSize) {
        ...
        for (int i = 0; i < cc.length; i++) {
            cur = cc[i];
            switch (mode) {
                case DVImportXml.InTag:
                    if (cur == RT) {
                        mode = DVImportXml.OutTag;
                        v.add(sb);
                        sb = new StringBuffer();
                    } else {
                        sb.append(cur);
                    ...
                    break;
                case DVImportXml.OutTag:
                    if (cur == LT) {
                        mode = DVImportXml.InTag;
                        v.add(sb);
                        sb = new StringBuffer();
                    } else {
                        sb.append(cur);
