 ...
 *  
 * <p>Title: DVXmlToDatavault</p>
 * <p>Description: Contains convenience methods to transfer XML data to/from a DVDataVault object instance.</p>
 *
 * @author Monte Seibel
 * @version 7.0
 * @formatter:on
...
public class DVXmlToDatavault {
	public static final String CLASSNAME = "DVXmlToDatavault";
	...
	public static final void extract(StringBuffer xml, DVDatavault dv, String dataVaultTag) {
		String col = Character.toString(DVXmlToDatavault.COLLECTION_DELIMITER);
		DVXmlToDatavault.extract(xml, dv, dataVaultTag, new StringBuffer(col));
	}

	public static final void extract(String xml, DVDatavault dv, String dataVaultTag) {
		StringBuffer stb = new StringBuffer(xml);
		String col = Character.toString(DVXmlToDatavault.COLLECTION_DELIMITER);
		DVXmlToDatavault.extract(stb, dv, dataVaultTag, new StringBuffer(col));
	}
	...
	private static final Vector _parse(StringBuffer xml, int vectorSize) {
		char cur;
		int mode = DVXmlToDatavault.OutTag;
		...
		for (int i = 0; i < cc.length; i++) {
			cur = cc[i];
			switch (mode) {
			case DVXmlToDatavault.InTag:
				if (cur == RT) {
					mode = DVXmlToDatavault.OutTag;
					v.add(sb);
				...
				break;
			case DVXmlToDatavault.OutTag:
				if (cur == LT) {
					mode = DVXmlToDatavault.InTag;
					v.add(sb);
					sb = new StringBuffer(100);
				} else {
					sb.append(cur);
